...
typedef struct Edge
{
    ...
    Edge(Platform platform, TrainTrack train_track) : platform(platform), train_track(train_track) {}
    Edge() {}
} Edge;
...
typedef struct Station
{
    ...
    string station_name;
    int popularity;
    tuple<Station *, Edge *> edges[6];

    Station(int station_id, string station_name, int popularity) : edges()
    {
    ...
    void run_traintrack(int tick)
    {
        for (int color = 0; color < 3; color++)
        {
            ...
            {
                Edge *edge = get<1>(edges[color * 2 + dir]);
                TrainTrack *train_track = &edge->train_track;
                if (edge == NULL || train_track->curr_train == NULL || train_track->curr_train->color != color)
                    continue;
                ...
                Station *next_station = get<0>(edges[color * 2 + dir]);
                if (get<0>(next_station->edges[color * 2 + dir]) == NULL)
                    train_track->curr_train->direction ^= 1; // flip direction
                Platform *next_platform = &get<1>(next_station->edges[color * 2 + train_track->curr_train->direction])->platform;
    ...
    void run_holdingarea(vector<Train> &train_threads)
    {
        for (int color = 0; color < 3; color++)
        {
            ...
            {
                Edge *edge = get<1>(edges[color * 2 + dir]);
                Platform *platform = &edge->platform;
                if (edge == NULL || platform->holding_area.empty())
                    continue;
    ...
    void run_platform()
    {
        for (int color = 0; color < 3; color++)
        {
            for (int dir = 0; dir < 2; dir++)
            {
                Edge *edge = get<1>(edges[color * 2 + dir]);
                Platform *platform = &edge->platform;
                if (edge == NULL || platform->curr_train == NULL || platform->curr_train->color != color)
                    continue;
