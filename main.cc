...
unordered_map<int, Platform> platforms;
unordered_map<int, TrainTrack> train_tracks;
...
void initialize_stations(size_t num_stations)
{
  ...
  for (int color = 0; color < 3; color++)
  {
    for (int i = 0; i < num_line_stations[color]; i++)
    {
      ...
      if (forward_id != -1)
      {
        station->next[color * 2] = &stations[forward_id];
        station->platforms[color * 2] = &platforms.at(curr_id * row_size + forward_id);
        station->train_tracks[color * 2] = &train_tracks.at(curr_id * row_size + forward_id);
      }
      if (backward_id != -1)
      {
        station->next[color * 2 + 1] = &stations[backward_id];
        station->platforms[color * 2 + 1] = &platforms.at(curr_id * row_size + backward_id);
        station->train_tracks[color * 2 + 1] = &train_tracks.at(curr_id * row_size + backward_id);
      }
    }
  }
}
  ...
  for (int src_id = 0; src_id < num_stations; src_id++)
  {
    for (int des_id = 0; des_id < num_stations; des_id++)
    {
      if (mat[src_id][des_id] > 0)
      {
        ...
        train_tracks.insert({src_id * row_size + des_id, TrainTrack(track_name, mat[src_id][des_id])});
        platforms.insert({src_id * row_size + des_id, Platform()});
      }
    }
  }
}
...
void spawn_trains_if_needed(int tick, int num_green_trains, int num_yellow_trains, int num_blue_trains, int num_line_stations[3], vector<Train> &train_threads)
{
  ...
  for (int color = 0; color < 3; color++)
  {
    ...
    for (int dir = 0; dir < to_create; dir++)
    {
      ...
      station->platforms[color * 2 + dir]->holding_area.emplace(WaitingTrain{curr_train_id, color, tick}); // error here
